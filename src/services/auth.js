...
export async function requestEmailVerification(email, name) {
  ...
  try {
      ...
      // NEW: Check if this email has a Salesforce account first
      let hasSalesforceAccount = false;
      let salesforceContactId = null;
      let salesforceAlcorId = null;
      let salesforceFirstName = null;
      let salesforceLastName = null;
      
      try {
          console.log("Checking for Salesforce account...");
          const salesforceCheck = await checkMemberAccount(email);
          
          console.log("Salesforce check result:", salesforceCheck);
          
          if (salesforceCheck.success && salesforceCheck.hasAccount && salesforceCheck.data) {
              hasSalesforceAccount = true;
              
              // Get the first account data (handle multiple accounts later if needed)
              const accountData = Array.isArray(salesforceCheck.data) ? salesforceCheck.data[0] : salesforceCheck.data;
              
              salesforceContactId = accountData.id || accountData.salesforceContactId;
              salesforceAlcorId = accountData.alcorId;
              salesforceFirstName = accountData.firstName;
              salesforceLastName = accountData.lastName;
              
              console.log("Found Salesforce account:", {
                  contactId: salesforceContactId,
                  alcorId: salesforceAlcorId,
                  name: `${salesforceFirstName} ${salesforceLastName}`
              });
          }
      } catch (sfError) {
          console.error("Error checking Salesforce account:", sfError);
      ...
      const result = await Promise.race([
          authCoreFn({ 
            ...
            email, 
            name,
            hasSalesforceAccount,  // NEW
            salesforceContactId,   // NEW
            salesforceAlcorId,     // NEW
            salesforceFirstName,   // NEW
            salesforceLastName     // NEW
          }),
