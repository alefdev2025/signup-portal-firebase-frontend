...
const InvoicesTab = ({ customerId = '4666' }) => {
  ...
  useEffect(() => {
    const checkAutopayStatus = async () => {
      if (!customerId || !SHOW_LEGACY_AUTOPAY_BANNER) {
  ...
  return (
    <div className="invoice-page -mx-6 -mt-6 md:mx-0 md:-mt-4 md:w-[95%] md:pl-4 min-h-screen" style={{ fontFamily: 'Helvetica, Arial, sans-serif' }}>
      ...
      {!loadingAutopayStatus && customerAutopayStatus && customerAutopayStatus.legacy?.autopayEnabled && !customerAutopayStatus.stripe?.autopayEnabled && SHOW_LEGACY_AUTOPAY_BANNER && (
        <LegacyAutopayBanner />
      )}
