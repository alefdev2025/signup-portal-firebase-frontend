...
const handleDownload = async (document) => {
  try {
    ...
    const fileExtension = lastDotIndex > -1 ? document.name.substring(lastDotIndex) : '';
    
    // Create the download filename using the formatted name + original extension
    const formattedName = formatDocumentName(document.name);
    const downloadFilename = formattedName + fileExtension;
    
    // Most common approach: Pass desired filename to the service
    // The backend should then set Content-Disposition header with this filename
    ...
    // Alternative approach if backend doesn't support custom filenames:
    // The backend should return the file with Content-Disposition header like:
    // Content-Disposition: attachment; filename="Medical Authorization Form.pdf"
    // using the document.name field instead of document.id
  ...
  try {
    ...
    reader.onload = async (e) => {
      try {
        ...
        // Prepend document type to filename
        const titleWithType = `${documentType}_${selectedFile.name}`;
        ...
        console.log('[DocumentsTab] Uploading document:', titleWithType);
    ...
    setUploading(false);
    document.getElementById('file-upload').value = ''; // Clear the file input regardless
  }
};
    ...
    {showDocumentTypeModal && ReactDOM.createPortal(
      <div className="fixed inset-0 z-[100] overflow-y-auto">
        ...
        <div className="flex min-h-full items-center justify-center p-4">
          <div className="relative bg-white rounded-2xl p-6 max-w-md w-full animate-fadeInUp">
            <h3 className="text-xl font-semibold text-gray-800 mb-4">Ask Sarah The Types Here</h3>
            <p className="text-sm text-gray-600 mb-6">Please select the type of document you're uploading. This helps us categorize your file properly.</p>
            
            <div className="space-y-3">
              <button
                onClick={() => setDocumentType('temp1')}
                className={`w-full p-3 rounded-lg border-2 text-left transition-all ${
                  documentType === 'temp1' 
                    ? 'border-[#6b5b7e] bg-purple-50 text-[#6b5b7e]' 
                    : 'border-gray-200 hover:border-gray-300'
                }`}
              >
                <span className="font-medium">Medical</span>
                <p className="text-sm text-gray-500 mt-1">Health records, prescriptions, physician documents</p>
              </button>
              
              <button
                onClick={() => setDocumentType('temp2')}
                className={`w-full p-3 rounded-lg border-2 text-left transition-all ${
                  documentType === 'temp2' 
                    ? 'border-[#6b5b7e] bg-purple-50 text-[#6b5b7e]' 
                    : 'border-gray-200 hover:border-gray-300'
              ...
              >
                ...
                <p className="text-sm text-gray-500 mt-1">Consent forms, directives, authorizations</p>
              </button>
              
              <button
                onClick={() => setDocumentType('temp3')}
                className={`w-full p-3 rounded-lg border-2 text-left transition-all ${
                  documentType === 'temp3' 
                    ? 'border-[#6b5b7e] bg-purple-50 text-[#6b5b7e]' 
                    : 'border-gray-200 hover:border-gray-300'
              ...
              >
                ...
                <p className="text-sm text-gray-500 mt-1">Insurance, payment records, funding documents</p>
              </button>
              
              <button
                onClick={() => setDocumentType('temp4')}
                className={`w-full p-3 rounded-lg border-2 text-left transition-all ${
                  documentType === 'temp4' 
                    ? 'border-[#6b5b7e] bg-purple-50 text-[#6b5b7e]' 
                    : 'border-gray-200 hover:border-gray-300'
              ...
              >
                ...
                <p className="text-sm text-gray-500 mt-1">Personal statements, preferences, other documents</p>
              </button>
            </div>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button
                ...
                className={`flex-1 px-4 py-2 rounded-lg transition-all ${
                  documentType 
                    ? 'bg-[#6b5b7e] text-white hover:bg-[#5a4a6d]' 
