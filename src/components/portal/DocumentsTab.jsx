...
const handleDownload = async (doc) => {
  ...
  const getDocumentIcon = (doc) => {
    return (
      <svg className="w-5 h-5 sm:w-6 sm:h-6 2xl:w-7 2xl:h-7 text-black" fill="none" stroke="currentColor" strokeWidth="0.5" viewBox="0 0 24 24">
      ...
      <div className="documents-tab -mx-6 -mt-6 md:mx-0 md:-mt-4 md:w-[95%] md:pl-4">
        ...
        <div className="flex items-center justify-center" style={{ minHeight: '30vh', marginTop: '10vh' }}>
          <div className="text-center">
            ...
            <p className="text-gray-500 font-light text-sm lg:text-base">Loading documents...</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="px-4 md:px-0">
        ...
        {documents.length === 0 ? (
          <div className="bg-white rounded-2xl p-8 sm:p-12 animate-fadeInUp animation-delay-100 border border-gray-200 text-center" style={{ boxShadow: '4px 6px 12px rgba(0, 0, 0, 0.08)' }}>
            <svg className="w-12 h-12 sm:w-16 sm:h-16 2xl:w-18 2xl:h-18 text-gray-400 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
            <p className="text-gray-800 text-sm sm:text-base 2xl:text-lg font-medium">No documents found</p>
            <p className="text-gray-500 text-[10px] sm:text-xs 2xl:text-sm mt-2 font-light">Your membership documents will appear here once uploaded by Alcor staff</p>
          </div>
        ) : (
          <>
            {/* Files Section */}
            <div className="bg-white rounded-2xl p-4 sm:p-8 animate-fadeInUp animation-delay-100 border border-gray-200" style={{ boxShadow: '4px 6px 12px rgba(0, 0, 0, 0.08)' }}>
              <h3 className="text-base sm:text-lg 2xl:text-xl font-semibold text-gray-800 mb-8 sm:mb-12 flex items-center gap-3 flex-wrap">
                <div className="bg-gradient-to-r from-[#0a1628] to-[#6e4376] p-2 sm:p-2.5 2xl:p-3 rounded-lg shadow-md">
                  <svg className="h-6 w-6 sm:h-8 sm:w-8 2xl:h-9 2xl:w-9 text-white" fill="none" stroke="currentColor" strokeWidth="1" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                  </svg>
                </div>
                ...
                {filteredDocuments.length > 0 ? (
                  filteredDocuments.map((doc, index) => (
                    <button
                      ...
                      className={`bg-white rounded-lg p-3 sm:p-3.5 2xl:p-4 border border-gray-200 shadow-sm flex items-center justify-between gap-3 animate-slideInRow hover:border-gray-300 hover:shadow-md transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed text-left w-full`}
                    ...
                    >
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="font-medium text-gray-800 text-[11px] sm:text-xs 2xl:text-sm truncate">{formatDocumentName(doc.name)}</h3>
                          {downloading[doc.id] && (
                            <div className="mt-1 flex items-center gap-2 text-[0.5rem] sm:text-[0.625rem] 2xl:text-xs text-purple-600">
                              <div className="w-2 h-2 sm:w-2.5 sm:h-2.5 2xl:w-3 2xl:h-3 relative">
                                <div className="absolute inset-0 rounded-full border-2 border-purple-100"></div>
                      ...
                      <div className="flex-shrink-0">
                        <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 2xl:w-5 2xl:h-5 text-gray-600 transition-all duration-200 hover:text-[#794384] hover:scale-110" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
        ...
        <div id="upload-section" className="mt-12 bg-white rounded-2xl p-4 sm:p-8 animate-fadeInUp animation-delay-200 overflow-hidden" style={{ boxShadow: '6px 8px 16px rgba(0, 0, 0, 0.1), -2px -2px 8px rgba(0, 0, 0, 0.04)' }}>
          ...
          <p className="text-gray-700 text-[11px] sm:text-sm 2xl:text-base leading-relaxed mb-8 sm:mb-12 max-w-2xl">
          ...
          {uploadSuccess && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6 animate-fadeIn">
              <div className="flex items-center gap-3">
                <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 2xl:w-5 2xl:h-5 text-green-600" fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                </svg>
                <p className="text-green-800 text-[11px] sm:text-xs 2xl:text-sm">Document uploaded successfully!</p>
              </div>
            </div>
          )}
          ...
          {error && error.includes('upload') && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
              <div className="flex items-center gap-3">
                <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 2xl:w-5 2xl:h-5 text-red-600" fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
                </svg>
                <p className="text-red-800 text-[11px] sm:text-xs 2xl:text-sm">{error}</p>
              </div>
            </div>
          )}
          
          <div>
            <label className="block text-[11px] sm:text-xs 2xl:text-sm font-medium text-[#6a5b8a] mb-2">
              Choose File
            </label>
            <input 
              type="file"
            ...
            <label 
              htmlFor="file-upload"
              className={`inline-flex items-center justify-center gap-3 px-4 sm:px-5 2xl:px-6 py-2 sm:py-2.5 2xl:py-3 bg-white border-2 border-gray-300 text-[#2a2346] rounded-2xl hover:border-[#6b5b7e] hover:text-[#6b5b7e] transition-all cursor-pointer text-[11px] sm:text-xs 2xl:text-sm ${
                uploading ? 'opacity-50 cursor-not-allowed' : ''
              }`}
            >
              {uploading ? (
                <>
                  <div className="animate-spin rounded-full h-3.5 w-3.5 sm:h-4 sm:w-4 2xl:h-5 2xl:w-5 border-b-2 border-[#6b5b7e]"></div>
                  <span>Uploading...</span>
                </>
              ) : (
                <>
                  <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 2xl:w-5 2xl:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <p className="text-[0.5rem] sm:text-[0.625rem] 2xl:text-xs text-[#6a5b8a] mt-4 mb-8">
        ...
        <div className="fixed inset-0 z-[100] overflow-y-auto">
          ...
          <div className="flex min-h-full items-center justify-center p-4">
            <div className="relative bg-white rounded-2xl p-7 2xl:p-8 sm:p-8 2xl:sm:p-10 max-w-lg w-full animate-fadeInUp shadow-xl">
              <h3 className="text-base sm:text-xl 2xl:text-2xl font-semibold text-gray-800 mb-6">Select Document Type</h3>
              <p className="text-[11px] sm:text-sm 2xl:text-base text-gray-600 mb-8">Please select the type of document you're uploading.</p>
              ...
              <div className="space-y-4">
                ...
                >
                  <span className="font-medium text-sm sm:text-base 2xl:text-lg">Membership Document</span>
                  <p className="text-[10px] sm:text-xs 2xl:text-sm text-gray-500 mt-2">Any documents related to your membership</p>
                </button>
                ...
                >
                  <span className="font-medium text-sm sm:text-base 2xl:text-lg">Profile Picture</span>
                  <p className="text-[10px] sm:text-xs 2xl:text-sm text-gray-500 mt-2">Your photo for identification purposes</p>
                </button>
              </div>
              ...
              <div className="flex gap-4 mt-10">
                ...
                <button
                  ...
                  className={`flex-1 px-4 sm:px-5 2xl:px-6 py-2 sm:py-2.5 2xl:py-3 rounded-xl transition-all text-[11px] sm:text-sm 2xl:text-base font-medium ${
                    documentType 
