...
const DocumentsTab2 = ({ contactId }) => {
  ...
  // Wider layout setting - matches MyInformationTab2
  const wider = true;

  // Define separator style - matches MyInformationTab2
  ...
  const mobileSeparator = () => (
    <div className="py-20 px-8 -mt-12 -mb-2">
      <div className="h-px rounded-full" style={{ background: 'linear-gradient(90deg, #4a5f7a 0%, #5a4f7a 40%, #7a5f8a 70%, #9e7398 100%)' }}></div>
    </div>
  );

  console.log('[DocumentsTab2] Component rendered with contactId:', contactId);

  // Add Helvetica font and animations matching MyInformationTab2
  useEffect(() => {
    ...
    style.innerHTML = `
      ...
      .documents-tab-2 .font-bold,
      .documents-tab-2 .font-semibold {
      ...
      .documents-tab-2 .font-medium {
        font-weight: 400 !important;
      }
      .documents-tab-2 .fade-in {
        animation: fadeIn 0.6s ease-out;
      }
      .documents-tab-2 .slide-in {
        animation: slideIn 0.6s ease-out;
      }
      .documents-tab-2 .slide-in-delay-1 {
        animation: slideIn 0.6s ease-out 0.1s both;
      }
      .documents-tab-2 .slide-in-delay-2 {
        animation: slideIn 0.6s ease-out 0.2s both;
      }
      .documents-tab-2 .slide-in-delay-3 {
  ...
  useEffect(() => {
    if (documentsLoaded && preloadedDocuments) {
      console.log('[DocumentsTab2] Using preloaded documents:', preloadedDocuments.length);
  ...
  useEffect(() => {
    console.log('[DocumentsTab2] useEffect triggered, contactId:', contactId, 'documentsLoaded:', documentsLoaded);
    if (!documentsLoaded && contactId) {
      loadDocuments();
    } else if (!contactId) {
      console.warn('[DocumentsTab2] No contactId provided');
      setLoading(false);
  ...
  const loadDocuments = async () => {
    try {
      ...
      console.log('[DocumentsTab2] Loading documents for contactId:', contactId);
      ...
      console.log('[DocumentsTab2] Raw result:', result);
      ...
      if (result.success && result.data) {
        ...
        const docs = Array.isArray(result.data) ? result.data : 
                     ...
                     [];
        console.log('[DocumentsTab2] Documents received:', docs);
        console.log('[DocumentsTab2] Documents count:', docs.length);
        console.log('[DocumentsTab2] First document:', docs[0]);
        ...
        console.log('[DocumentsTab2] Grouped documents:', grouped);
        setGroupedDocs(grouped);
      } else {
        console.error('[DocumentsTab2] Failed to load documents:', result.error);
        setError(result.error || 'Failed to load documents');
      }
    } catch (err) {
      console.error('[DocumentsTab2] Error loading documents:', err);
    ...
    try {
      ...
      reader.onload = async (e) => {
        try {
          ...
          console.log('[DocumentsTab2] Uploading document:', file.name);
          ...
          if (response.ok && responseData.success) {
            console.log('[DocumentsTab2] Upload successful!');
          ...
          console.error('[DocumentsTab2] Error:', err);
      ...
      console.error('[DocumentsTab2] Error uploading document:', err);
  ...
  // Container classes that change based on wider setting - matches MyInformationTab2
  const containerClasses = wider 
  ? "documents-tab-2 w-full -mx-10 -mt-6 md:-mt-4"
  : "documents-tab-2 -mx-6 -mt-6 md:mx-0 md:-mt-4 md:w-11/12 md:pl-4";
      ...
      <div className="hidden sm:block">
        ...
        <div className={`slide-in-delay-${++sectionIndex}`}>
          <div className={`${wider ? 'px-2 py-6' : 'px-1 py-4'}`}>
            ...
            {documents.length === 0 ? (
              <div className="bg-white rounded-2xl p-8 sm:p-12 border border-gray-200 text-center" style={{ boxShadow: '4px 6px 12px rgba(0, 0, 0, 0.08)' }}>
              ...
              <div className="bg-white rounded-2xl p-4 sm:p-8 border border-gray-200" style={{ boxShadow: '4px 6px 12px rgba(0, 0, 0, 0.08)' }}>
                  ...
                  {filteredDocuments.length > 0 ? (
                    filteredDocuments.map((doc, index) => (
                      <button
                        ...
                        className={`bg-gray-50 rounded-lg p-3 outline outline-1 outline-gray-200 shadow-sm flex items-center justify-between gap-3 hover:bg-gray-100 hover:outline-gray-300 hover:shadow-md transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed text-left w-full`}
                      >
        ...
        <div className={`slide-in-delay-${++sectionIndex}`}>
          <div className={`${wider ? 'px-2' : 'px-1'}`}>
            <div id="upload-section" className="bg-white rounded-2xl p-4 sm:p-8 overflow-hidden" style={{ boxShadow: '6px 8px 16px rgba(0, 0, 0, 0.1), -2px -2px 8px rgba(0, 0, 0, 0.04)' }}>
      ...
      <div className="sm:hidden">
        ...
        <div className={`slide-in-delay-${++sectionIndex} mx-4`}>
          <div className="p-2 py-4">
            ...
            {documents.length === 0 ? (
              <div className="bg-white rounded-2xl p-8 border border-gray-200 text-center" style={{ boxShadow: '4px 6px 12px rgba(0, 0, 0, 0.08)' }}>
                <svg className="w-18 h-18 text-gray-400 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
                <p className="text-gray-800 text-lg font-medium">No documents found</p>
              ...
              <div className="bg-white rounded-2xl p-4 border border-gray-200" style={{ boxShadow: '4px 6px 12px rgba(0, 0, 0, 0.08)' }}>
                <h3 className="text-xl font-semibold text-gray-800 mb-8 flex items-center gap-3 flex-wrap">
                  <div className="bg-[#2a1b3d] p-3.5 rounded-lg">
                    <svg className="h-7 w-7 text-white" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
        ...
        <div className={`slide-in-delay-${++sectionIndex} mx-4`}>
          <div className="p-2">
            <div id="upload-section" className="bg-white rounded-2xl p-4 overflow-hidden" style={{ boxShadow: '6px 8px 16px rgba(0, 0, 0, 0.1), -2px -2px 8px rgba(0, 0, 0, 0.04)' }}>
              <div className="h-4"></div>
              <h2 className="text-xl font-medium text-[#2a2346] mb-6 tracking-widest">UPLOAD FILES</h2>
              <p className="text-gray-700 text-base leading-relaxed mb-8">
                Upload new documents to your member portal. Files are stored with your membership file for Alcor staff access.
              </p>
