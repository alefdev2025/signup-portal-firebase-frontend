...
const styleConfig = {
  ...
  separator: {
    wrapper: "h-px sm:h-0.5 my-8 sm:my-16 mx-0 sm:ml-4 sm:mx-0 w-auto sm:w-1/3 rounded-full bg-gray-300 sm:bg-gray-400",
  ...
  display: {
    ...
    grid: {
      single: "space-y-4",
      twoColumn: "grid grid-cols-1 gap-x-4 gap-y-4 md:grid-cols-2 md:gap-x-6",
      threeColumn: "grid grid-cols-1 gap-x-4 gap-y-4 md:grid-cols-2 lg:grid-cols-3 md:gap-x-6",
      fullSpan: "md:col-span-2",
      tripleSpan: "md:col-span-2 lg:col-span-3"
    },
    // Definition list styles
    dl: {
      wrapper: "grid grid-cols-1 gap-x-4 gap-y-4 md:gap-x-6",
 ...
 export const getSectionIconStyles = (sectionType) => {
  const iconClass = styleConfig.sectionIcons[sectionType] || styleConfig.header.iconContainer;
  return {
    container: iconClass,
    icon: styleConfig.header.icon
  };
 };
 
 // Helper function to check if a section has an edit button
 export const sectionHasEditButton = (sectionName) => {
  return styleConfig.editableConfig.sectionsWithEditButton[sectionName] || false;
 };
 ...
 export const isFieldVisibleInEditMode = (sectionName, fieldName) => {
  const sectionFields = styleConfig.editableConfig.visibleInEditMode[sectionName];
 ...
 export const isFieldEditable = (sectionName, fieldName) => {
  ...
  const sectionFields = styleConfig.editableConfig.editableFields[sectionName];
