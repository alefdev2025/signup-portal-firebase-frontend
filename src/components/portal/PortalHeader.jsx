...
const NotificationBell = ({ activeTab, setActiveTab, isGlassmorphic, variant = 'regular' }) => {
  ...
  const getIcon = (type) => {
    ...
    switch(type) {
      case 'message':
        return (
          <svg className={iconSize} fill="none" stroke="#794384" strokeWidth="1.5" viewBox="0 0 24 24">
  ...
  return (
    <div className="relative">
      ...
      >
        <svg className={bellIconSize} fill="none" stroke="#f7cd46" strokeWidth="1.5" viewBox="0 0 24 24" style={{filter: 'drop-shadow(0 0 1px #794384)'}}>
      ...
      {/* Dropdown */}
      {isOpen && (
        <div className={`notification-dropdown absolute right-0 mt-2 w-80 md:w-96 ${isGlassmorphic ? 'bg-white' : 'bg-white'} rounded-lg shadow-xl border ${isGlassmorphic ? 'border-gray-200' : 'border-gray-200'} z-[9999]`}>
          <div className={`px-4 py-3 border-b ${isGlassmorphic ? 'border-gray-200' : 'border-gray-200'} flex items-center justify-between`}>
            <div className="flex items-center gap-0.5">
              <h3 className={`font-semibold ${isGlassmorphic ? 'text-gray-900' : 'text-gray-900'}`} style={{ fontFamily: 'Helvetica, Arial, sans-serif !important' }}>Notifications</h3>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <p className={`text-sm font-medium ${!notification.read ? 'font-semibold' : ''} ${isGlassmorphic ? 'text-gray-900' : 'text-gray-900'}`} style={{ fontFamily: 'Helvetica, Arial, sans-serif' }}>
                          {notification.title}
                        </p>
                        {!notification.read && (
                      ...
                      <p className={`text-sm mt-0.5 ${isGlassmorphic ? 'text-gray-600' : 'text-gray-600'}`} style={{ fontFamily: 'Helvetica, Arial, sans-serif' }}>{notification.content}</p>
                      <p className={`text-xs mt-1 ${isGlassmorphic ? 'text-gray-400' : 'text-gray-400'}`} style={{ fontFamily: 'Helvetica, Arial, sans-serif' }}>{formatNotificationTime(notification.createdAt)}</p>
                    </div>
                  </div>
                </div>
              ))
          ...
          <div className={`px-4 py-3 border-t ${isGlassmorphic ? 'border-gray-200' : 'border-gray-200'}`}>
            <button 
              ...
              className={`w-full text-center text-sm font-medium ${isGlassmorphic ? 'text-[#5b2f4b] hover:text-[#3f2541]' : 'text-[#5b2f4b] hover:text-[#3f2541]'} flex items-center justify-center gap-1`}
              style={{ fontFamily: 'Helvetica, Arial, sans-serif' }}
            >
...
const PortalHeader = ({ 
  ...
  const searchIconColor = "text-[#f7cd46]";
    ...
    <header className={headerClasses}>
      <div className="flex items-center justify-between h-full">
        ...
        <div className="flex items-center gap-4 flex-1">
          ...
          >
            <svg className="w-6 h-6" fill="none" stroke="#794384" viewBox="0 0 24 24">
    ...
    >
      {getBreadcrumb()}
          ...
          <div className="hidden md:block relative ml-auto mr-8">
            ...
            />
            <svg className={`absolute right-3 top-1/2 transform -translate-y-1/2 ${searchIconSize} ${searchIconColor}`} fill="none" stroke="#794384" strokeWidth="1.5" viewBox="0 0 24 24" style={{filter: 'drop-shadow(0 0 0.5px #794384)'}}>
...
<span className="md:hidden">Emergency</span>
          ...
          <button className={settingsButtonClasses}>
            <svg className={settingsIconSize} fill="none" stroke="#794384" strokeWidth="1.5" viewBox="0 0 24 24">
