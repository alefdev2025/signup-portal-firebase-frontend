...
const navigationItems = [
    { 
      id: 'overview', 
      label: 'Home', 
      icon: ( <svg className="w-7 h-7" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg> ) 
    },
    { 
      id: 'account', 
      label: 'Account', 
      icon: ( <svg className="w-7 h-7" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 3c1.66 0 3 1.34 3 3s-1.34 3-3 3-3-1.34-3-3 1.34-3 3-3zm0 14.2c-2.5 0-4.71-1.28-6-3.22.03-1.99 4-3.08 6-3.08 1.99 0 5.97 1.09 6 3.08-1.29 1.94-3.5 3.22-6 3.22z"/></svg> ),
      subItems: [
    ...
    { 
      id: 'membership', 
      label: 'Membership', 
      icon: ( <svg className="w-7 h-7" fill="currentColor" viewBox="0 0 24 24"><path d="M20 4H4c-1.11 0-1.99.89-1.99 2L2 18c0 1.11.89 2 2 2h16c1.11 0 2-.89 2-2V6c0-1.11-.89-2-2-2zm0 14H4v-6h16v6zm0-10H4V6h16v2z"/></svg> ),
      subItems: [
    ...
    { 
      id: 'documents', 
      label: 'Documents', 
      icon: ( <svg className="w-7 h-7" fill="currentColor" viewBox="0 0 24 24"><path d="M14,2H6A2,2 0 0,0 4,4V20A2,2 0 0,0 6,22H18A2,2 0 0,0 20,20V8L14,2M18,20H6V4H13V9H18V20Z"/></svg> ),
      subItems: [
    ...
    { 
      id: 'payments', 
      label: 'Payments', 
      icon: ( <svg className="w-7 h-7" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg> ),
      subItems: [
...
const PortalSidebar = ({ 
  ...
  // Medium width - halfway between w-[280px] md:w-[320px] and w-[240px] md:w-[260px]
  const sidebarWidth = 'w-[260px] md:w-[290px]';
      ...
      {layoutMode === 'traditional' ? (
                  ...
                  {navigationItems.map((item) => (
                    <div key={item.id} className="group">
                      <button
                        ...
                        className={`w-full flex items-center justify-between px-4 py-4 rounded-lg transition-all font-normal relative group ${
        ...
        >
          ...
          <div className="relative z-10 flex flex-col h-full">
              ...
              <div className="space-y-1">
                {navigationItems.map((item) => (
                  <div key={item.id} className="group">
                    <button
                      onClick={() => handleNavClick(item.id)}
                                              className={`w-full flex items-center justify-between px-3 py-3.5 rounded-lg transition-all font-normal relative group ${
                        isItemActive(item.id)
                          ? 'bg-white/10 text-white'
                          : 'text-white/80 hover:text-white hover:bg-white/5'
                      }`}
                      ...
                      <div className="mt-1 ml-11 space-y-1">
                        {item.subItems.map((subItem) => (
                          <button
                            key={subItem.id}
                            onClick={() => handleSubItemClick(item.id, subItem.id)}
                                                          className={`w-full text-left px-3 py-2.5 rounded-md transition-all text-base relative group ${
                              activeTab === `${item.id}-${subItem.id}`
                                ? 'bg-white/10 text-white'
                                : 'text-white/70 hover:text-white hover:bg-white/5'
                            }`}
            ...
            <div className="p-4 border-t border-white/20">
              <div className="flex items-center gap-3 px-3 py-2">
                <div className="w-9 h-9 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center flex-shrink-0 shadow-lg">
                  {profileImage ? (
                    <img src={profileImage} alt="Profile" className="w-full h-full rounded-full object-cover" />
                  ) : (
