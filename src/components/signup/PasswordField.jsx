...
const PasswordField = ({ 
  ...
  return (
    <div className={`${className}`}>
      <label htmlFor={id} className={labelClassName || "block text-gray-800 text-base font-medium mb-2"}>
        {label}
      </label>
      ...
      <div className="relative">
        <input 
          ...
          className={inputClassName || `w-full px-4 py-3 bg-white border ${strength.isStrong && !error ? 'border-green-500' : 'border-gray-300'} rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 text-gray-800 text-base pr-12`}
      ...
      {strength.isStrong && !error && (
        <div className="flex items-center mt-2">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-green-500 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
          </svg>
          <span className="text-green-600 text-xs">Strong password</span>
        </div>
      )}
      ...
      {showRequirements && !strength.isStrong && (
        <>
          ...
          <div className="bg-gray-50 p-2 rounded-md border border-gray-200 mb-2 text-xs">
      ...
      {error && <p className={errorClassName || "text-red-500 text-xs mt-1"}>{error}</p>}
    </div>
  );
};
