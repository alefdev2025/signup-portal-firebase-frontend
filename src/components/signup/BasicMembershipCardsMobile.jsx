...
const BasicMembershipCardsMobile = ({ packageInfo, animationComplete, content }) => {
  ...
  return (
    <div className="mb-16 md:hidden px-4">
      <div className="space-y-10">
        ...
        <div className={`rounded-2xl overflow-hidden shadow-md transition-all duration-700 ease-in-out delay-150 transform ${animationComplete ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          {/* Light Top Section */}
          <div className="bg-white p-8 pb-8">
            <div className="flex items-center mb-8">
              <div className="p-3 rounded-lg shadow-md mr-4 transform transition duration-300" style={{ 
          ...
          <div className={`${content.cards.whatHappensNext.bgGradient} text-white p-8 pt-8 pb-10`}>
            <h5 className="text-lg font-semibold mb-8 text-white">Future Funding Options:</h5>
            <div className="flex flex-col space-y-8">
              {content.cards.whatHappensNext.futureOptions.map((option, index) => (
                <div key={index} className="flex items-start">
                  <div className="bg-[#454575] p-2.5 rounded-lg mr-4 flex-shrink-0 mt-0.5">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={option.icon} />
                    </svg>
                  </div>
                  <span className="text-base text-gray-200 leading-relaxed">{option.text}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className={`rounded-2xl overflow-hidden shadow-md transition-all duration-700 ease-in-out delay-300 transform ${animationComplete ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          {/* Dark Gradient Bottom Section */}
          <div className={`${content.cards.fundingNotice.bgGradient} text-white p-8 pt-8 pb-10`}>
            <h5 className="text-lg font-semibold text-white mb-8">What's Included:</h5>
            <div className="space-y-6">
              {content.cards.fundingNotice.includedFeatures.map((feature, index) => (
                <div key={index} className="flex items-start">
                  <img src={alcorStar} alt="Star" className="w-5 h-5 mr-3 mt-0.5 filter brightness-0 invert flex-shrink-0" />
