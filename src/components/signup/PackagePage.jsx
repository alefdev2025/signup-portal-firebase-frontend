...
import { PackageContentMobile } from "./PackageContentMobileDark";
...
export default function PackagePage({ onNext, onBack, initialData = {}, preloadedMembershipData = null, isParentReady = true }) {
  ...
  const OriginalVersionOptionCard = ({ option, index }) => {
    ...
    return (
      ...
      >
        <div className={`relative rounded-lg md:rounded-[2rem] overflow-hidden shadow-md ${
          selectedOption === option ? "md:ring-2 md:ring-[#775684]" : ""
        } transition-all duration-300 hover:shadow-lg transform hover:scale-[1.01] h-full flex flex-col`}>
          
          {/* Selected badge for mobile - top right corner */}
          {selectedOption === option && (
            <div className="md:hidden absolute top-3 right-3 z-10 bg-[#775684] text-white px-3 py-1 rounded-md text-sm font-medium">
              SELECTED
            </div>
          )}
          
          {/* Use Desktop View for all screen sizes */}
    ...
    <div className="w-full bg-gray-100" style={{
      ...
      <div className="w-full mx-auto px-4 sm:px-8 py-8" style={{ maxWidth: "1100px" }}>
        <div className="mb-4 sm:mb-6 md:mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 px-4 sm:px-6 md:px-0">
            {/* NEURO OPTION */}
...
<div className="mt-6 mx-4 sm:mx-6 md:hidden">
  <div className="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
    {/* Header with navy background */}
    <div className="p-5" style={{ backgroundColor: '#1a2342' }}>
      <h3 className="text-white text-lg font-normal text-center" style={{ fontFamily: SYSTEM_FONT }}>
        Your Selected Package
      </h3>
    </div>
    
    {/* Content section */}
    <div className="p-5" style={{ fontFamily: SYSTEM_FONT }}>

      {/* Selected option display */}
      <div className="flex items-center justify-between mb-6">
        <span className="text-gray-600 text-base">Package Type:</span>
        <div className="flex items-center">
          <span className="font-medium text-gray-900 text-base">
            {getSelectedOptionName()}
          </span>
          {/* Small icon next to the name */}
          <div className={`ml-2 p-1.5 rounded-full ${
            selectedOption === "neuro" ? 'bg-[#454575]' : 
            selectedOption === "wholebody" ? 'bg-[#293253]' : 
            'bg-[#1c324c]'
          }`}>
      ...
      {/* Pricing breakdown with divider */}
      <div className="border-t border-gray-200 pt-5">
        {/* Preservation cost */}
        <div className="flex justify-between items-center mb-3">
          <span className="text-gray-600 text-base">Preservation:</span>
          <span className="font-medium text-gray-900 text-base">
            {selectedOption === "basic" ? "Not required" : `$${calculatePreservationEstimate(selectedOption)?.toLocaleString()}`}
          </span>
        </div>
        
        {/* Membership cost */}
        <div className="flex justify-between items-center">
          <span className="text-gray-600 text-base">Membership:</span>
          <span className="font-medium text-gray-900 text-base">
            ${Math.round(parseFloat(getPackagePrice("standard")))}/year
          </span>
        </div>
        
        {/* Subtext for preservation options */}
        {selectedOption !== "basic" && (
          <div className="mt-3 pt-3 border-t border-gray-100">
            <p className="text-xs text-gray-500 text-center italic">
              Typically funded with life insurance at $25-$250/month
            </p>
          </div>
        )}
      </div>
      
      {/* Simple text instruction to scroll up */}
      <div className="mt-5 pt-3 text-center">
        <p className="text-[#775684] text-sm">
          ↑ Scroll up to change selection
        </p>
      </div>
    </div>
  </div>
          ...
          <div className={`px-4 sm:px-6 md:px-0 transform transition-all duration-700 ease-in-out ${
            showCards ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
