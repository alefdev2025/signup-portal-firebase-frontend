...
export const PackageContentMobile = ({ 
  ...
  return (
    <div 
      ...
      className={`cursor-pointer ${USE_UPDATED_VERSION ? 'transform transition duration-300 hover:scale-[1.02]' : 'h-full'}`} 
      style={{...fadeInStyle, ...getAnimationDelay(option === "neuro" ? 0 : option === "wholebody" ? 1 : 2)}}
    >
      <div className={`${USE_UPDATED_VERSION ? 'rounded-2xl md:rounded-t-[2rem] md:rounded-b-3xl' : 'rounded-lg md:rounded-[2rem]'} overflow-hidden shadow-md ${selectedOption === option ? (USE_UPDATED_VERSION ? "ring-2 ring-[#775684]" : "ring-2 ring-[#775684]") : (USE_UPDATED_VERSION ? "ring-1 ring-gray-400" : "")} transition-all duration-300 ${USE_UPDATED_VERSION ? '' : 'hover:shadow-lg transform hover:scale-[1.01] flex flex-col h-full'}`}>
        ...
        <div className={`md:hidden bg-white ${USE_UPDATED_VERSION ? 'p-4 sm:p-5 border-t border-gray-200' : 'p-5'}`} style={{ fontFamily: SYSTEM_FONT }}>
