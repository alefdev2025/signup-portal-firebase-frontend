...
const SIGNUP_STEPS = [
  ...
  { id: 'completion', label: 'Complete Membership', component: 'MembershipCompletionStep' }
];
...
export const SignupFlowProvider = ({ children }) => {
  ...
  const expectedNextStep = useRef(null);
  ...
  useEffect(() => {
    if (initialStepSet || isLoading || !currentUser || !signupState) {
      return;
    }
    ...
    // Check if we're expecting a specific next step (from goToNextStep)
    if (expectedNextStep.current !== null) {
      const expected = expectedNextStep.current;
      expectedNextStep.current = null; // Clear it
      
      // If backend matches our expectation, we're good
      if (progress === expected) {
        log(`Backend confirmed navigation to step ${progress}`);
  ...
  const getMaxAllowedStep = () => {
    ...
    // If we're expecting a next step, allow it
    if (expectedNextStep.current !== null) {
      return Math.max(expectedNextStep.current, userProgress + 1);
    }
    
    // Otherwise allow current progress + 1
    return Math.min(userProgress + 1, SIGNUP_STEPS.length - 1);
  };
  ...
  const value = {
    ...
    goToNextStep: () => {
      ...
      if (nextStep < SIGNUP_STEPS.length) {
        // Mark what step we expect the backend to update to
        expectedNextStep.current = nextStep;
        
        // Clear expectation after 5 seconds
        setTimeout(() => {
          expectedNextStep.current = null;
        }, 5000);
