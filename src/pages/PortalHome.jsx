import React, { useState } from 'react';
import alcorLogo from '../assets/images/alcor-white-logo.png';
import dewarsImage from '../assets/images/dewars2.jpg';

// Portal Navigation Icons - matching the mockup style
const ProfileIcon = () => (
  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
    <path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/>
  </svg>
);

const DocumentsIcon = () => (
  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
    <path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z"/>
  </svg>
);

const MembershipIcon = () => (
  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
    <path d="M20 4H4c-1.11 0-1.99.89-1.99 2L2 18c0 1.11.89 2 2 2h16c1.11 0 2-.89 2-2V6c0-1.11-.89-2-2-2zm0 14H4v-6h16v6zm0-10H4V6h16v2z"/>
  </svg>
);

const PlanIcon = () => (
  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
    <path d="M19 3h-4.18C14.4 1.84 13.3 1 12 1c-1.3 0-2.4.84-2.82 2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-7 0c.55 0 1 .45 1 1s-.45 1-1 1-1-.45-1-1 .45-1 1-1zm2 14H7v-2h7v2zm3-4H7v-2h10v2zm0-4H7V7h10v2z"/>
  </svg>
);

// Help Panel Component
const HelpPanel = ({ showHelpInfo, toggleHelpInfo, helpItems }) => {
  return (
    <>
      {/* Help Toggle Button - Always Visible */}
      <button
        onClick={toggleHelpInfo}
        className={`fixed right-8 bottom-20 z-[100] bg-[#C49278] text-white p-4 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110 ${
          showHelpInfo ? 'ring-4 ring-white ring-opacity-50' : ''
        }`}
      >
        {showHelpInfo ? (
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
          </svg>
        ) : (
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
        )}
      </button>

      {/* Help Panel - Conditionally Rendered */}
      {showHelpInfo && (
        <div className="fixed right-5 bottom-40 z-[99] bg-white rounded-xl shadow-2xl w-80 max-h-[60vh] overflow-y-auto border border-gray-200">
          <div className="bg-[#C49278] text-white p-4 rounded-t-xl sticky top-0 z-10">
            <h3 className="font-semibold text-lg flex items-center">
              <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              Help & Information
            </h3>
          </div>
          <div className="p-4">
            {helpItems.map((item, index) => (
              <div key={index} className="mb-4 pb-4 border-b border-gray-100 last:border-b-0">
                <h4 className="font-semibold text-gray-900 mb-2 text-sm">{item.title}</h4>
                <div className="text-sm text-gray-600 leading-relaxed">
                  {typeof item.content === 'string' ? item.content : item.content}
                </div>
              </div>
            ))}
          </div>
        </div>
      )}
    </>
  );
};

const PortalPage = () => {
  // State for help panel
  const [showHelpInfo, setShowHelpInfo] = useState(false);

  // Toggle help panel
  const toggleHelpInfo = () => {
    setShowHelpInfo(prev => !prev);
  };

  // Help content for member portal
  const portalHelpContent = [
    {
      title: "Member Portal Overview",
      content: "Welcome to your Alcor member portal. Here you can manage your profile, access important documents, schedule appointments, and stay connected with your membership benefits."
    },
    {
      title: "My Profile",
      content: "Update your personal information, emergency contacts, and membership preferences. Keep your profile current to ensure we can reach you when needed."
    },
    {
      title: "Documents",
      content: "Access and download important membership documents, contracts, and educational materials. All your essential paperwork in one secure location."
    },
    {
      title: "Calendar",
      content: "View upcoming events, schedule consultations, and manage appointments. Stay informed about Alcor activities and important dates."
    },
    {
      title: "Messages",
      content: "Communicate securely with Alcor staff, receive important updates, and manage your correspondence history."
    },
    {
      title: "Payments",
      content: "View payment history, update billing information, and manage your membership dues. Secure payment processing through Stripe."
    },
    {
      title: "Need assistance?",
      content: (
        <span>
          Our support team is here to help. Contact us at{' '}
          <a href="mailto:support@alcor.com" className="text-[#775684] hover:underline">
            support@alcor.com
          </a>{' '}
          or call (480) 905-1906.
        </span>
      )
    }
  ];

  const marcellusStyle = {
    fontFamily: "'Marcellus', 'Marcellus Pro Regular', serif"
  };

  const portalSections = [
    { name: 'My Profile', icon: <ProfileIcon /> },
    { name: 'Documents', icon: <DocumentsIcon /> },
    { name: 'Membership', icon: <MembershipIcon /> },
    { name: 'My Plan', icon: <PlanIcon /> }
  ];

  // Define gradient colors for each card - RAINBOW gradients from the example
  const cardGradients = [
    'linear-gradient(135deg, #785683 0%, #443660 50%, #162740 100%)', // Profile - Purple to navy
    'linear-gradient(135deg, #B98168 0%, #785683 50%, #443660 100%)', // Documents - Peach to purple to dark
    'linear-gradient(135deg, #C4A23F 0%, #B98168 50%, #785683 100%)', // Membership - Gold to peach to purple
    'linear-gradient(135deg, #785683 0%, #443660 50%, #162740 100%)'  // Plan - Purple to navy (same as Profile)
  ];

  return (
    <div style={marcellusStyle}>
      {/* Simplified Header - Logo Only */}
      <header className="absolute top-0 left-0 right-0 z-20 px-4 md:px-6 py-4 flex justify-between items-center">
        <img src={alcorLogo} alt="Alcor" className="h-14 md:h-20" />
        
        {/* Logout button with profile icon */}
        <button className="flex items-center space-x-3 text-white hover:opacity-70 transition-opacity">
          <svg className="w-14 h-14" fill="white" viewBox="0 0 24 24">
            <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 3c1.66 0 3 1.34 3 3s-1.34 3-3 3-3-1.34-3-3 1.34-3 3-3zm0 14.2c-2.5 0-4.71-1.28-6-3.22.03-1.99 4-3.08 6-3.08 1.99 0 5.97 1.09 6 3.08-1.29 1.94-3.5 3.22-6 3.22z"/>
          </svg>
          <span className="font-light text-xl text-white">Logout</span>
        </button>
      </header>

      {/* Full Height Hero Section with Icons Overlaid */}
      <div className="relative min-h-screen overflow-hidden">
        <div 
          className="absolute inset-0 bg-cover bg-center"
          style={{ backgroundImage: `url(${dewarsImage})` }}
        />
        <div 
          className="absolute inset-0"
          style={{ background: 'radial-gradient(ellipse at center 85%, rgba(220, 194, 84, 0.96) 0%, rgba(185, 129, 104, 0.96) 15%, rgba(127, 96, 131, 0.96) 35%, rgba(85, 57, 108, 0.96) 55%, rgba(36, 44, 73, 0.95) 75%, rgba(27, 40, 66, 0.94) 90%, rgba(18, 36, 59, 0.93) 100%)' }}
        />
        
        {/* Content overlaid on image */}
        <div className="relative z-10 min-h-screen flex flex-col">
          {/* Hero text section */}
          <div className="flex-1 flex flex-col items-center justify-center px-4 md:px-8 text-center pt-32 pb-20">
            <h1 className="text-3xl md:text-5xl font-light text-white mb-3">
              Member Portal
            </h1>
            <p className="text-lg md:text-xl text-white/80 font-light">
              Access your membership benefits and resources
            </p>
          </div>
          
          {/* White background section for icons */}
          <div className="bg-white" style={{ minHeight: '500px' }}>
            <div className="px-4 md:px-8 py-8">
              <div className="max-w-7xl mx-auto">
                <h2 className="text-2xl md:text-3xl font-light text-gray-800 mb-8">Member Portal Home</h2>
                
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                  {portalSections.map((section, index) => (
                    <div 
                      key={index}
                      className="cursor-pointer group h-full"
                    >
                      <div className="rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1 bg-white shadow-lg h-full flex flex-col">
                        {/* Top white section matching the preservation cards */}
                        <div className="bg-white p-5 flex-grow">
                          {/* Icon with background matching preservation cards */}
                          <div className="flex items-center gap-3 mb-3">
                            <div 
                              className="p-2.5 rounded-lg"
                              style={{ 
                                background: index === 0 ? '#7F6083' : // Purple
                                           index === 1 ? '#1a2342' : // Dark blue
                                           index === 2 ? '#11243a' : // Darker blue
                                           '#7F6083' // Purple
                              }}
                            >
                              <div className="text-white">
                                {React.cloneElement(section.icon, { className: "w-5 h-5" })}
                              </div>
                            </div>
                            <h3 className="text-xl font-semibold text-gray-900">{section.name}</h3>
                          </div>
                          
                          {/* Description */}
                          <p className="text-gray-600 text-sm leading-relaxed">
                            {index === 0 ? 'Manage your personal information and emergency contacts.' :
                             index === 1 ? 'View and download important membership documents.' :
                             index === 2 ? 'Review your membership status and benefits.' :
                             'Access your preservation plan details.'}
                          </p>
                        </div>
                        
                        {/* Bottom gradient section - "What's Included" style - FIXED MEMBERSHIP GRADIENT */}
                        <div 
                          className="px-5 py-5"
                          style={{ 
                            background: index === 0 ? 'linear-gradient(135deg, #785683 0%, #443660 50%, #162740 100%)' : // Profile - Neuro gradient
                                       index === 1 ? 'linear-gradient(135deg, #162740 0%, #443660 70%, #785683 100%)' : // Documents - Whole body gradient
                                       index === 2 ? 'linear-gradient(135deg, #443660 0%, #162740 30%, #443660 100%)' : // Membership - Basic gradient (corrected)
                                       'linear-gradient(135deg, #785683 0%, #443660 50%, #162740 100%)' // Plan - Neuro gradient
                          }}
                        >
                          <h4 className="text-white font-semibold mb-3 text-sm">Quick Actions:</h4>
                          <div className="space-y-2 text-white text-sm">
                            {index === 0 && (
                              <>
                                <div className="flex items-center">
                                  <span className="mr-3 text-xs">✦</span>
                                  <span>Update Contact Info</span>
                                </div>
                                <div className="flex items-center">
                                  <span className="mr-3 text-xs">✦</span>
                                  <span>Emergency Contacts</span>
                                </div>
                              </>
                            )}
                            {index === 1 && (
                              <>
                                <div className="flex items-center">
                                  <span className="mr-3 text-xs">✦</span>
                                  <span>View Contracts</span>
                                </div>
                                <div className="flex items-center">
                                  <span className="mr-3 text-xs">✦</span>
                                  <span>Download Forms</span>
                                </div>
                              </>
                            )}
                            {index === 2 && (
                              <>
                                <div className="flex items-center">
                                  <span className="mr-3 text-xs">✦</span>
                                  <span>Membership Status</span>
                                </div>
                                <div className="flex items-center">
                                  <span className="mr-3 text-xs">✦</span>
                                  <span>Member Benefits</span>
                                </div>
                              </>
                            )}
                            {index === 3 && (
                              <>
                                <div className="flex items-center">
                                  <span className="mr-3 text-xs">✦</span>
                                  <span>Plan Details</span>
                                </div>
                                <div className="flex items-center">
                                  <span className="mr-3 text-xs">✦</span>
                                  <span>Coverage Info</span>
                                </div>
                              </>
                            )}
                          </div>
                        </div>
                      </div>
                    </div>
                  ))}
                </div>

                {/* Quick Access Buttons */}
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mt-10">
                  <button className="relative overflow-hidden rounded-lg px-5 py-3 shadow-sm hover:shadow-md transition-all duration-200 group">
                    <div 
                      className="absolute inset-0 bg-cover bg-center"
                      style={{ backgroundImage: `url(${dewarsImage})` }}
                    />
                    <div 
                      className="absolute inset-0"
                      style={{ background: 'linear-gradient(135deg, rgba(147, 116, 151, 0.9) 0%, rgba(127, 96, 131, 0.9) 50%, rgba(107, 76, 111, 0.9) 100%)' }}
                    />
                    <div className="relative z-10">
                      <h4 className="text-lg font-medium text-white mb-1">Quick Start Guide</h4>
                      <p className="text-white/90 text-sm">Learn how to make the most of your membership</p>
                    </div>
                  </button>
                  
                  <button className="relative overflow-hidden rounded-lg px-5 py-3 shadow-sm hover:shadow-md transition-all duration-200 group">
                    <div 
                      className="absolute inset-0 bg-cover bg-center"
                      style={{ backgroundImage: `url(${dewarsImage})` }}
                    />
                    <div 
                      className="absolute inset-0"
                      style={{ background: 'linear-gradient(135deg, rgba(107, 76, 111, 0.9) 0%, rgba(85, 57, 108, 0.9) 50%, rgba(60, 50, 90, 0.95) 100%)' }}
                    />
                    <div className="relative z-10">
                      <h4 className="text-lg font-medium text-white mb-1">Member Benefits</h4>
                      <p className="text-white/90 text-sm">Explore all the advantages of your membership</p>
                    </div>
                  </button>
                  
                  <button className="relative overflow-hidden rounded-lg px-5 py-3 shadow-sm hover:shadow-md transition-all duration-200 group">
                    <div 
                      className="absolute inset-0 bg-cover bg-center"
                      style={{ backgroundImage: `url(${dewarsImage})` }}
                    />
                    <div 
                      className="absolute inset-0"
                      style={{ background: 'linear-gradient(135deg, rgba(60, 50, 90, 0.95) 0%, rgba(36, 44, 73, 0.95) 50%, rgba(18, 36, 59, 0.95) 100%)' }}
                    />
                    <div className="relative z-10">
                      <h4 className="text-lg font-medium text-white mb-1">Support Center</h4>
                      <p className="text-white/90 text-sm">Get help when you need it most</p>
                    </div>
                  </button>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>

      {/* Help Panel Component - Always rendered so pink button is visible */}
      <HelpPanel 
        showHelpInfo={showHelpInfo} 
        toggleHelpInfo={toggleHelpInfo} 
        helpItems={portalHelpContent} 
      />
    </div>
  );
};

export default PortalPage;